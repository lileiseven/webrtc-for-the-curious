...
Sender includes its local time `sendertime1` in SR.
When receiver gets SR packet it sends back RR.
Among other things RR includes `sendertime1` just received from sender.
There's a delay between receiving SR and sending RR, because of that RR also includes "delay since last sender report" time `DLSR`. 
`DLSR` is used to adjust round-trip time estimate later on in the process.
Once sender receives RR it subtracts `sendertime1` and `DLSR` from current time `sendertime2`.
This time delta is called round-trip propagation delay or round-trip time.

`rtt = sendertime2 - sendertime1 - DLSR`

Round-trip time in plain English: 
- I send you a message with my clock's reading (say it is 4:20pm 42seconds and 420 milliseconds). 
- You send me this very time back. 
- You also include the time it took you from reading my message to sending the message back (5 milliseconds).
...
- Therefore round-trip time is 265 msec.
